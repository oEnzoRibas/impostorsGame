    ...
    <PageContainer>
      ...
      >
        <Card style={{
            ...
            <p style={{ 
                margin: 0, fontSize: theme.fontSize.s, color: "rgba(255,255,255,0.8)", 
                textTransform: "uppercase", letterSpacing: "2px", fontWeight: "bold" 
            }}>
                SUA IDENTIDADE
            </p>
            
            <div style={{ marginTop: theme.spacing.m, minHeight: "60px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                {showSecret ? (
                    <div style={{ animation: "fadeIn 0.3s" }}>
                        {isImpostor ? (
                            <>
                                <span style={{ fontSize: "2rem", display: "block" }}>🕵️‍♂️</span>
                                <span style={{ fontSize: theme.fontSize.l, fontWeight: "bold", color: "#fff" }}>VOCÊ É O IMPOSTOR</span>
                            </>
                        ) : (
                            <>
                                <span style={{ fontSize: theme.fontSize.l, fontWeight: "bold", color: "#fff", display: "block" }}>
                                    🤫 Palavra: {mySecret?.word}
                                </span>
                                <span style={{ fontSize: theme.fontSize.s, color: "rgba(255,255,255,0.9)" }}>
                                    Tema: {mySecret?.theme}
                                </span>
                            </>
                        )}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "5px" }}>
                        ...
                        <span style={{ fontSize: theme.fontSize.s, color: "rgba(255,255,255,0.6)" }}>Toque para revelar</span>
                    </div>
                )}
            </div>
        </Card>
      ...
      <div style={{ textAlign: "center", marginBottom: theme.spacing.l }}>
        <h2 style={{ margin: 0, fontSize: "1.5rem", color: theme.colors.text.primary }}>
            Rodada {room.currentRound} <span style={{ color: theme.colors.text.disabled }}>/ {room.maxRounds}</span>
        </h2>
        <p style={{ color: isMyTurn ? theme.colors.accent : theme.colors.text.secondary, fontWeight: isMyTurn ? "bold" : "normal" }}>
            {isMyTurn 
      ...
      {/* --- GRID DE JOGADORES --- */}
      <div style={{ 
          display: "flex", flexWrap: "wrap", justifyContent: "center", gap: theme.spacing.l, 
        ...
        {room.players.map((player) => {
          ...
          const lastWord = hasWords ? safeWordsList[safeWordsList.length - 1] : "";
          const isPlayerTurn = player.id === room.turnPlayerId;

          return (
            <div key={player.id} style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              {hasWords && (
                <div
                  ...
                  onClick={() => setHoveredPlayerId(isHovered ? null : player.id)} // Funciona no touch também
                  style={{
                    position: "absolute",
                    bottom: "100%",
                    marginBottom: "10px",
                ...
                >
                  {isHovered ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                       <span style={{ fontSize: "10px", color: "#888", borderBottom: "1px solid #eee" }}>HISTÓRICO</span>
                       {safeWordsList.map((w, i) => (
                           <span key={i} style={{ color: i === safeWordsList.length -1 ? "#000" : "#666" }}>
                               {w}
                           </span>
                       ))}
                    </div>
              ...
              <div style={{
                ...
                border: isPlayerTurn 
                    ...
                    : (player.id === me.id ? `3px solid ${theme.colors.primary}` : `3px solid transparent`), // Verde se sou eu
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: "28px", fontWeight: "bold",
                boxShadow: isPlayerTurn ? `0 0 15px ${theme.colors.accent}60` : "none",
                transition: "all 0.3s"
              }}>
                {player.name.charAt(0).toUpperCase()}
              </div>

              <span style={{ marginTop: "8px", fontSize: theme.fontSize.s, fontWeight: "bold", color: isPlayerTurn ? theme.colors.accent : theme.colors.text.primary }}>
                {player.name}
              </span>
            </div>
          );
      ...
      {isMyTurn && (
        <div style={{
            ...
            <Card style={{ 
                ...
                flexDirection: "row", display: "flex", gap: "10px", alignItems: "center",
